...
/** Base trait for Cucumber test runners using Playwright.
  *
  * This trait provides access to Playwright page objects and test data. The lifecycle hooks
  * (BeforeAll, AfterAll, After) are defined in the CucumberLifecycleHooks object to satisfy
  * Cucumber's requirement for static hooks to be in a static context.
  */
