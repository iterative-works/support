...
trait CommonStepDefinitions:
    ...
    When("^I fill in \"([^\"]*)\" with \"([^\"]*)\"$") { (field: String, value: String) =>
        val input = page.locator(
            s"input[name='$field'], input[id='$field'], input[placeholder='$field']"
        ).first()
        input.fill(value)
    }
    ...
    When("^I check \"([^\"]*)\"$") { (field: String) =>
        val checkbox = page.locator(
            s"input[type='checkbox'][name='$field'], input[type='checkbox'][id='$field']"
        ).first()
        checkbox.check()
    }
    ...
    Then("^I should be on the \"([^\"]*)\" page$") { (pageName: String) =>
        ...
        assert(
            ...
            s"Expected to be on $expectedPath but was on $currentUrl"
        )
    }
    ...
    Then("^the \"([^\"]*)\" field should contain \"([^\"]*)\"$") {
        (field: String, expectedValue: String) =>
            ...
            assert(
                ...
                s"Expected '$expectedValue' but got '$actualValue'"
            )
    }
