...
case class ParamMap(params: Seq[(String, String)]) {
  ...
  def nonEmpty(key: String): Validation[String, String] = for {
    v <- required(key)
  ...
  def boolean(key: String): Validation[String, Boolean] = for {
    v <- nonEmpty(key)
    result <-
      if (v == "true") Validation.succeed(true) else Validation.succeed(false)
  } yield result

  def int(key: String): Validation[String, Int] = for {
    v <- nonEmpty(key)
    result <- Validation
      .fromTry(Try(v.toInt))
      .asError(
...
object PaymentResult {
  def fromParams(
    ...
    val configValidation = for {
      ...
      _ <-
        if ((config.matchCredentials _).tupled(received)) Validation.unit
