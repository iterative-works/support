...
package works.iterative.sqldb.mysql.migration

import zio.*
import works.iterative.core.Language
import works.iterative.sqldb.MessageCatalogueRepository
import works.iterative.sqldb.mysql.*

object MySQLMessageCatalogueMigrationCLI extends ZIOAppDefault:
            ...
            exitCode <- config match
                ...
                case None =>
                    Console.printLineError(
                        "Invalid arguments. Use --help for usage instructions."
                    ) *>
    ...
    private def runMigration(config: CliConfig): ZIO[Scope, Any, ExitCode] =
        val program =
            for
                _ <- Console.printLine(
                    s"Starting migration for language '${config.language}' from '${config.resourcePath}'"
                )
                ...
                lang <- ZIO.attempt(Language.unsafe(config.language))
                    .mapError(e =>
                        new RuntimeException(s"Invalid language code: ${config.language}", e)
                    )
                _ <- if config.dryRun then
                    Console.printLine(
                        s"[DRY RUN] Would migrate messages from ${config.resourcePath} for language $lang"
                    )
                else
                    MySQLMessageCatalogueMigration.migrateFromJson(
                        repository,
                        lang,
                        config.resourcePath
                    )
        ...
        program
            .provideSomeLayer[Scope](
                MySQLDatabaseSupport.layer >>> MySQLMessageCatalogueRepository.layer
            )
            .catchAll { error =>
...
end MySQLMessageCatalogueMigrationCLI
