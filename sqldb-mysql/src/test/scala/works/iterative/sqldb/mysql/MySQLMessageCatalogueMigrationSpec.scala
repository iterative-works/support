// PURPOSE: Tests for MySQLMessageCatalogueMigration to verify JSON to SQL migration functionality
...
import works.iterative.sqldb.mysql.migration.MySQLMessageCatalogueMigration

object MySQLMessageCatalogueMigrationSpec extends ZIOSpecDefault:

  def spec = suite("MySQLMessageCatalogueMigrationSpec")(
    test("migrateFromJson successfully migrates 5 sample messages") {
      for
        ...
        _ <- MySQLMessageCatalogueMigration.migrateFromJson(
          repository,
          Language.EN,
          "/test_messages.json"
        )
    ...
    test("migrateFromJson fails with descriptive error for missing resource") {
      for
        ...
        _ <- migrationService.migrate()
        repository <- ZIO.service[MessageCatalogueRepository]
        result <- MySQLMessageCatalogueMigration.migrateFromJson(
          repository,
          Language.EN,
          "/nonexistent.json"
        ).either
    ...
    MySQLMessageCatalogueRepository.layer,
