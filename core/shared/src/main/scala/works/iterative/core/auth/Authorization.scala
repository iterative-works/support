...
/** Authorization helper object providing declarative guards for permission checking.
  *
  * NOTE: This is application-layer infrastructure that orchestrates effects.
  * Pure authorization domain logic lives in PermissionLogic.
  *
  * Usage patterns:
  * {{{
...
object Authorization:
  ...
  /** Require permission before executing effect, fail with Forbidden if denied.
    *
    * This method enforces fail-closed behavior: if permission check fails or returns false,
    * the effect is not executed and an AuthenticationError.Forbidden is raised.
    *
    * @param op The permission operation to check
    * @param target The target resource
    * @param effect The effect to execute if permission granted
    * @return ZIO effect that succeeds only if permission granted
    */
  def require[R, E, A](
      op: PermissionOp,
    ...
    for {
      ...
      result <- if (allowed) effect
                else ZIO.fail(AuthenticationError.Forbidden(target.value, op.value))
    } yield result

  /** Check if current user has permission on target resource.
    *
    * This method returns a Boolean indicating whether permission is granted.
    * Unlike `require`, this does not fail - it returns false when permission is denied.
    *
    * Note: PermissionService.isAllowed returns UIO[Boolean] (cannot fail), so this
    * method also cannot fail. The typed error channel provides future extensibility
    * if we need to add fallible permission checks.
    *
    * @param op The permission operation to check
    * @param target The target resource
    * @return ZIO[CurrentUser & PermissionService, AuthenticationError, Boolean]
    */
  def check(
      op: PermissionOp,
      target: PermissionTarget
  ...
  /** Execute effect only if permission granted, return None if denied.
    *
    * This method filters the effect result based on permission. If permission is denied,
    * it returns None without executing the effect. If permission is granted, it executes
    * the effect and returns its result.
    *
    * @param op The permission operation to check
    * @param target The target resource
    * @param effect The effect that returns Option[A]
    * @return ZIO effect that returns None if permission denied, otherwise effect result
    */
  def withPermission[R, E, A](
    ...
    for {
      ...
      permService <- ZIO.service[PermissionService]
      allowed <- permService.isAllowed(currentUser, op, target)
      result <- if (allowed) effect else ZIO.succeed(None)
    } yield result
  ...
  /** Filter a list of resources to only those the current user has permission to access.
    ...
    * Instead of checking permission N times (once per resource), it makes a single query
    * to get all allowed resource IDs, then filters the list locally.
    *
    ...
    *
    * @param op The permission operation to check
    * @param items The list of resources to filter
    * @param extractTarget Function to extract PermissionTarget from each resource
    * @return ZIO effect returning filtered list of permitted resources
    */
  def filterAllowed[A](
      op: PermissionOp,
      items: Seq[A]
