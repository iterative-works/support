...
/** Service for accessing message catalogues by language.
  *
  * This service provides a layer-based approach to managing message catalogues, allowing different
  ...
  *   - SQL-based (backend/dynamic): Messages pre-loaded from database at startup with hot reload
  *     support
  *
  * Choose JSON when:
  ...
  *
  * @see
  *   [[works.iterative.sqldb.SqlMessageCatalogueService]] for SQL implementation with reload
  *   support
  */
