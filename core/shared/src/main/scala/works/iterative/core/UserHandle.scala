...
/** A simple object to hold unique user identification together with "humane" identification Usually
  * we need to display the user's name in the UI, which we should be able to do without resorting to
  * external services. This handle is to be used in all user-generated content, like comments,
  * reviews, etc.
  */
