...
/** A relation tuple represents a relationship between a user and a target resource.
  *
  * In Zanzibar-inspired ReBAC (Relationship-Based Access Control), permissions are derived from
  * relation tuples. Each tuple states that a specific user has a specific relationship with a
  * target resource.
  *
  * @param user
  *   The user who has the relation (subject)
  * @param relation
  *   The type of relationship (e.g., "owner", "editor", "viewer")
  * @param target
  *   The resource that the relation applies to (object)
  *
  * Example:
  * {{{
