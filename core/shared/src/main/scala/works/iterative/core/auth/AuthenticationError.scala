...
/** Authentication and authorization error types.
  *
  * This enum defines all possible authentication and authorization failures that can occur in the
  * system. Using Scala 3 enum ensures exhaustive pattern matching and provides type-safe error
  * handling.
  *
...
enum AuthenticationError extends Exception:
    /** User is not authenticated (no valid session or token).
      ...
      *
      * @param message
      *   User-facing message with translation ID
      */
...
object AuthenticationError:
    ...
    /** Helper to create an InvalidToken error for JWT issues.
      *
      * @param reason
      *   The reason for the invalid JWT (included in message args)
      */
