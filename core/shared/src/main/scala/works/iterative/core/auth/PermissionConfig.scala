 ...
 *
 * Defines which relations imply other relations. For example, an "owner"
 * relation might imply "editor" and "viewer" relations.
 *
 * @param implications Map from relation to the set of relations it implies
 *
 * Example:
 * {{{
 *   NamespaceConfig(
 ...
 *
 * This configuration is used by the permission system to compute which permissions
 * are granted based on stored relation tuples. It implements a Zanzibar-inspired
 * model where permissions can be derived through relation inheritance.
 *
 * @param namespaces Map from namespace name to its configuration
 *
 * Example:
 * {{{
 *   PermissionConfig(
...
case class PermissionConfig(
  ...
  /**
   * Maximum depth for permission inheritance traversal.
   * This prevents DoS attacks from deeply nested or circular permission hierarchies.
   */
   ...
   *
   * @param namespace The resource namespace (e.g., "document", "folder")
   * @param relation The starting relation (e.g., "owner", "editor")
   * @return Set of all relations granted by the input relation
   *
   * Example:
   * {{{
   ...
   *
   * Uses breadth-first traversal to find all implied relations, respecting
   * the maximum depth limit to prevent excessive computation.
   *
   * @param start The starting relation
   * @param implications The implication map for this namespace
   * @param maxDepth Maximum depth to traverse
   * @return Set of all relations reachable from start (including start itself)
   */
