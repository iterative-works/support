...
/** Audit event representing a security-relevant action.
  *
  * All security events (permission checks, authentication, etc.) are recorded as audit events for
  * compliance, security monitoring, and forensics.
  *
  * @param timestamp
  *   When the event occurred
  * @param userId
  *   The user involved (None for unauthenticated events)
  * @param eventType
  *   Type of event (e.g., "permission_check", "login", "logout")
  * @param resource
  *   The resource involved (for permission checks)
  * @param action
  *   The action performed (for permission checks)
  * @param result
  *   Event result: "allowed", "denied", "success", "failure", "error"
  * @param reason
  *   Optional reason for the result
  * @param metadata
  *   Additional context (IP address, user agent, error details, etc.)
  */
...
/** Service for logging security audit events.
  *
  * All implementations must be fail-safe: logging failures should never interrupt normal
  * application flow. Use UIO (uninterruptible IO) to ensure audit logging cannot cause application
  * failures.
  *
...
trait AuditLogService:
    ...
    /** Log a permission check event.
      *
      * Records whether a user was allowed or denied access to a resource. Should be called for
      * EVERY permission check, regardless of result.
      *
      * @param userId
      *   The user whose permission was checked
      * @param resource
      ...
      * @param reason
      *   Optional reason for denial (e.g., "No relation", "Database error")
      * @return
      *   UIO effect that logs the event (never fails)
      */
    ...
    /** Log an authentication event.
      ...
      *
      * @param userId
      *   The authenticated user (None for failed authentication)
      * @param eventType
      *   Event type: "login", "logout", "login_failed", etc.
      * @param success
      *   true if successful, false if failed
      * @param metadata
      *   Additional context (IP, user agent, error details, etc.)
      * @return
      *   UIO effect that logs the event (never fails)
      */
