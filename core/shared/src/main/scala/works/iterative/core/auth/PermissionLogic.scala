...
/** Pure domain logic for permission checking.
  *
  * This object contains the functional core of the permission system following FCIS (Functional
  * Core, Imperative Shell) principles. All functions here are:
  *   - Pure (no side effects)
  *   - Deterministic (same inputs always produce same outputs)
  *   - Testable without ZIO or any infrastructure
  *
  * The effect-based orchestration (ZIO, database access, etc.) belongs in PermissionService
...
object PermissionLogic:
    ...
    /** Checks if a user is allowed to perform an action on a target resource.
      ...
      *   1. Finding all relation tuples for the user and target 2. For each relation, computing all
      *      implied permissions using the config 3. Checking if the requested action is in the set
      *      of granted permissions
      *
      * @param userId
      *   The user requesting access
      * @param action
      *   The permission being checked (e.g., "view", "edit", "delete")
      * @param target
      *   The target resource (e.g., "document:123")
      * @param tuples
      *   All relation tuples in the system
      * @param config
    ...
    /** Lists all object IDs in a namespace that a user can access with a given action.
      *
      * This is a pure function that performs a reverse lookup: given a user and an action, find all
      * resources they can access. This is essential for efficient authorization-aware queries
      * (e.g., "list all documents this user can edit").
      *
      * @param userId
      *   The user whose access we're checking
      * @param action
      *   The permission being checked (e.g., "view", "edit")
      * @param namespace
      *   The resource namespace to search (e.g., "document", "folder")
      * @param tuples
      *   All relation tuples in the system
      * @param config
      *   Permission configuration defining inheritance rules
      * @return
      *   Set of object IDs the user can access
      *
      * Example:
      * {{{
      *   val tuples = Set(
