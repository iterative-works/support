...
/** In-memory implementation of AuditLogService for testing.
  *
  * This implementation stores audit events in a ZIO Ref for verification in tests. Events are kept
  ...
  *
  * @param events
  *   Ref holding the list of audit events
  */
class InMemoryAuditLogService(events: Ref[List[AuditEvent]]) extends AuditLogService:
    ...
    /** Get all audit events (for testing).
      *
      * Returns events in reverse chronological order (newest first). This method is only for
      * testing - production implementations should not expose this method.
      */
