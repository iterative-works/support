...
/** Service for recording application metrics.
  ...
  * Implementations may send metrics to monitoring systems (Prometheus, Datadog, etc.) or be no-op
  * for testing/development.
  *
  * Metric types:
  *   - Counter: Monotonically increasing value (e.g., request count, error count) Use for events
  *     that only increase: requests processed, errors encountered, items created
  *   - Timer: Duration measurements (e.g., request latency, operation duration) Use for measuring
  *     how long operations take: API response time, database query duration
  *   - Gauge: Point-in-time value that can go up or down (e.g., queue size, active connections) Use
  *     for current state values: memory usage, queue depth, number of active sessions
  *
...
trait MetricsService:
    /** Record an increment to a counter metric.
      ...
      *
      * @param name
      *   Metric name (use dot notation: "auth.login.success")
      * @param tags
      *   Optional key-value pairs for metric dimensions
      */
    ...
    /** Record a duration measurement.
      ...
      *
      * @param name
      *   Metric name (use dot notation: "permission.check.duration")
      * @param duration
    ...
    /** Record a point-in-time value.
      ...
      *
      * @param name
      *   Metric name (use dot notation: "cache.size")
      * @param value
      *   The current value
      * @param tags
      *   Optional key-value pairs for metric dimensions
