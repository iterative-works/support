...
/** In-memory implementation of PermissionService using ZIO Ref for state management.
  ...
  *
  * NOTE: This is application-layer infrastructure (uses ZIO effects and manages state). Pure domain
  * logic lives in PermissionLogic.
  *
  * The service delegates all permission checking logic to PermissionLogic, ensuring that business
  * rules are pure and testable independently of the effect system.
  *
  * @param storage
  *   Ref holding the set of relation tuples
  * @param config
  *   Permission configuration defining namespace-specific inheritance rules
  */
class InMemoryPermissionService(
    ...
    /** Check if a user is allowed to perform an action on a target resource.
      *
      * This method delegates to PermissionLogic.isAllowed (pure function) for the actual permission
      * checking logic. This maintains separation between effects (this layer) and pure domain logic
      * (PermissionLogic).
      *
      * @param subj
      ...
      *   The target resource
      * @return
      *   UIO[Boolean] - true if allowed, false otherwise (fail-closed)
      */
    def isAllowed(
    ...
    /** List all resource IDs in a namespace that the user is allowed to access.
      *
      * This is a reverse lookup operation useful for authorization-aware queries. For example:
    ...
    /** Grant a permission relation to a user.
      ...
      * @return
      *   UIO[Boolean] - always true for in-memory implementation
      */
    def grantPermission(
        userId: UserId,
        relation: String,
...
object InMemoryPermissionService:
    /** Create an InMemoryPermissionService with the given configuration.
      *
      * @param config
      *   Permission configuration defining namespace rules
      * @return
      *   ZIO effect that creates the service
      */
