...
trait DetailComponentsModule:
  ...
  def details: DetailComponents

  trait DetailComponents:
    def section(
...
trait DefaultDetailComponentsModule(using ctx: ComponentContext)
    extends DetailComponentsModule:
  self: IconsModule =>

  override val details: DetailComponents = new DetailComponents:
    ...
    override def files(fs: List[FileRef]): HtmlElement =
      ul(
        role := "list",
    ...
    override def file(f: FileRef): HtmlElement =
      li(
