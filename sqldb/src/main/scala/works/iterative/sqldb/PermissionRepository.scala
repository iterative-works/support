...
/** Repository interface for persisting RelationTuples to a SQL database.
  *
  * This trait defines the contract for storing Zanzibar-inspired permission tuples,
  * enabling database-backed PermissionService implementations.
  *
...
trait PermissionRepository:
  ...
  /** Check if a specific relation exists between user and target.
    ...
    * @return Task[Boolean] - true if relation exists, false otherwise
    */
  def hasRelation(
      userId: UserId,
      relation: String,
  ...
  /** Add a relation tuple to the database.
    *
    * This operation should be idempotent - adding the same relation twice should succeed
    * without error (either by using INSERT IGNORE or catching unique constraint violations).
    *
  ...
  /** Get all relation tuples for a user in a specific namespace.
    *
    * This is the primary query for permission checking - fetches all relations
    * the user has within a namespace, which are then evaluated by PermissionLogic.
    *
    * @param userId The user ID
    * @param namespace The resource namespace to filter by
    * @return Task[Set[RelationTuple]] - Set of all relation tuples for the user in namespace
    */
