...
/** Database-backed implementation of PermissionService with fail-closed error handling.
  ...
  *
  * SECURITY CRITICAL: Database errors must NEVER bypass authorization checks. The fail-closed
  * pattern ensures that any infrastructure failure (database down, connection timeout, query error)
  * results in access denial rather than granting unintended permissions.
  *
  * @param repository
...
case class DatabasePermissionService(
    ...
    /** Check if a user is allowed to perform an action on a target resource.
      *
      * This method:
      *   1. Fetches all relation tuples for the user in the target's namespace 2. Delegates to
      *      PermissionLogic.isAllowed (pure function) 3. Returns false (fail-closed) if any
      *      database error occurs 4. Logs warnings when database errors prevent permission checks
      *
      * @param subj
    ...
    /** List all resource IDs in a namespace that the user is allowed to access.
      *
      * This method fetches all relation tuples for the user in the namespace, then delegates to
      * PermissionLogic.listAllowed to compute accessible resources.
      *
    ...
    /** Grant a permission relation to a user.
      *
      * Persists the relation tuple to the database via repository.addRelation. The repository
      * implementation should be idempotent (duplicate grants succeed).
      *
      * @param userId
    ...
    /** Revoke a permission relation from a user.
      *
      * Removes the relation tuple from the database via repository.removeRelation. The repository
      * implementation should be idempotent (revoking non-existent relations succeeds).
      *
      * @param userId
