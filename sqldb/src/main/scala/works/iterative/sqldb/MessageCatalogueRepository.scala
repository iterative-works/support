...
trait MessageCatalogueRepository:
    /** Retrieves all message catalogue entries for a specific language. Used for initial load and
      * reload of message catalogue from database.
      *
      * @param language
    ...
    /** Inserts multiple message catalogue entities in a single transaction. Used for migration from
      * JSON files to database. All inserts succeed or all fail (all-or-nothing transaction).
