...
end ConsulKeyValueStore

object ConsulKeyValueStore:
    val layer: URLayer[
        ConsulToken & BaseUri & CustomTapir.BackendProvider,
        StringKeyValueStore
    ] =
        ZLayer {
            for
                ...
                backend <- ZIO.service[CustomTapir.BackendProvider]
            yield ConsulKeyValueStore(token, baseUri, backend.get)
        }

    val fromEnv: ZLayer[CustomTapir.BackendProvider, SecurityException, StringKeyValueStore] =
        ConsulToken.fromEnv >>> ZLayer {
            for
                token <- ZIO.service[ConsulToken]
                backend <- ZIO.service[CustomTapir.BackendProvider]
                addr <- System.envOrElse("CONSUL_ADDR", "http://localhost:8500")
            yield ConsulKeyValueStore(token, BaseUri(addr), backend.get)
        }
end ConsulKeyValueStore
